        ...
        {annotationsOpen && (
          <aside className="w-80 shrink-0 bg-[#FAF7F0] border-l border-[#e8e6df] flex flex-col z-10 transition-all duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {/* New Comment Input */}
              {activeTarget && (
                <div className="bg-white border border-[#49704F]/50 ring-2 ring-[#49704F]/20 rounded-2xl p-4 shadow-sm relative z-20 mb-6 slide-in-from-top-2 animate-in duration-200">
              ...
              {annotations.map(ann => (
                <div key={ann.id} className="relative group">
                  <div className="absolute -left-2 top-0 w-6 h-6 rounded-full border-2 border-white bg-[#49704F] text-white flex items-center justify-center font-bold text-[10px] z-10 transition-colors">{ann.id}</div>
            ...
            <div className="p-4 border-t border-[#e8e6df]">
              <button 
                onClick={() => {
                  setAnnotationsOpen(true);
                  // By default select block-1 if none is active when clicking New Annotation
